...
- "If you wanted to add a new skill called `my-workflow` to the `fe-workflows` plugin, list every file you'd need to create or modify." (Expected: SKILL.md, permissions.yaml, skill-routing.yaml, plugin.json version bump, marketplace.json version bump)
