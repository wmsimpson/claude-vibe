...
```bash
SCHEDULER_PY="$(find ~/.claude/plugins/cache -path '*/macos-scheduler/*/skills/macos-scheduler/resources/scheduler_manager.py' | head -1)"
```

Make the task wrapper executable (one-time):

```bash
WRAPPER_SH="$(find ~/.claude/plugins/cache -path '*/macos-scheduler/*/skills/macos-scheduler/resources/job_wrapper.sh' | head -1)"
chmod +x "$WRAPPER_SH"
```
