...
```
~/.claude/plugins/cache/claude-vibe/shared-resources/<version>/resources/
```

Other skills reference them with a glob pattern (same pattern used for `markdown_to_gdocs.py`, `google_auth.py`, etc.):

```bash
python3 ~/.claude/plugins/cache/claude-vibe/shared-resources/*/resources/fiscal_calendar.py
```
...
A resource belongs in `shared-resources` when:
...
4. In consumer skills, reference via: `~/.claude/plugins/cache/claude-vibe/shared-resources/*/resources/<file>`
